namespace MGroup.Tests.DDM.PFetiDP
{
	...
	public static class PFetiDPSolverTests3DSmall
	{
		...
		public static void Run(double stiffnessRatio, EnvironmentChoice env, MatrixFormat matrixFormat)
		{
			...
			Assert.InRange(numIterations, 1, 11);
		}
			...
			IProcessingEnvironment environment, MatrixFormat matrixFormat)
		{
			...
			foreach (ISubdomain subdomain in model.Subdomains)
			{
				var corners = CornerNodeUtilities.FindCornersOfBrick3D(subdomain);
				foreach (INode node in corners)
				{
					if ((node.GetMultiplicity() > 1) && (node.Constraints.Count == 0))
					{
			...
			//// Plot model
			//PlotDecomposition(model, cornerDofs);
		...
		//private static void PlotDecomposition(IStructuralModel model, ICornerDofSelection cornerDofsSelection)
		//{
		//	string plotDirectory = @"C:\Users\Serafeim\Desktop\PFETIDP\Example4x4x4\";
		//	var writer = new FetiDpWriter(plotDirectory, cornerDofsSelection, true);
		//	var mesh = new VtkMesh(model.Nodes, model.Elements);
		//	writer.PlotGlobalMesh(mesh);
		//	writer.PlotSubdomains(model);
		//}
	}
}
