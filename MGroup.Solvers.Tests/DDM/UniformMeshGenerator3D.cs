		...
		private CellConnectivity<TNode>[] CreateElements(TNode[] allVertices)
		{
			...
			for (int i = 0; i < cellsPerX; ++i)
			{
				for (int j = 0; j < cellsPerY; ++j)
				{
					...
					{
						int cell = i * cellsPerZ * cellsPerY + j * cellsPerZ + k;
						int firstVertex = i * verticesPerZ * verticesPerY + j * verticesPerZ + k;
						var verticesOfCell = new int[]                                 // Node order for Hexa8
						{
							firstVertex + verticesPerZ * verticesPerY + verticesPerZ + 1,   // ( 1,  1,  1)
							firstVertex + verticesPerZ + 1,									// (-1,  1,  1)
							firstVertex + 1,												// (-1, -1,  1)
							firstVertex + verticesPerZ * verticesPerY + 1,                  // ( 1, -1,  1)

							firstVertex + verticesPerZ * verticesPerY + verticesPerZ,		// ( 1,  1, -1)
							firstVertex + verticesPerZ,										// (-1,  1, -1)
							firstVertex + 0,												// (-1, -1, -1)
							firstVertex + verticesPerZ * verticesPerY,						// ( 1, -1, -1)
						};
