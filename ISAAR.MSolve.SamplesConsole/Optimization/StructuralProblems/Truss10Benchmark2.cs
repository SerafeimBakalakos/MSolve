...
{
    class Truss10Benchmark2 : DesignProblem
    {
        public Truss10Benchmark2()
        {
            this.Dimension = 10;
        ...
        private class Truss10Design : IDesign
        {
            ...
            private void CreateLogs(Model model, LinearAnalyzer childAnalyzer)
            {
                ...
                childAnalyzer.LogFactories[1] = new LinearAnalyzerLogFactory(trackedDOFs,
            ...
            private double[] EvaluateConstraints(LinearAnalyzer childAnalyzer)
            {
                ...
                constraints.AddLast(max - maxDisplacement);

                // Stresses
                double[] minStresses = { -1e6, -1e6, -1e6, -1e6, -1e6, -1e6, -1e6, -1e6, -1e6, -1e6 };
                double[] maxStresses = { 1e6,   1e6, 1e6,  1e6,  1e6,  1e6,  1e6,  1e6,  1e6,  1e6 };
