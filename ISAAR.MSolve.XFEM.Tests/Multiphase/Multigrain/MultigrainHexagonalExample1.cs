...
{
    public static class MultigrainHexagonalExample1
    {
        ...
        //private const double hexagonSize = 0.01;
        //private const int numHexagonsX = 150, numHexagonsY = 150;
        //private const double minHexGridX = minX - 2 * hexagonSize - 0.03, maxHexGridY = maxY + 2 * hexagonSize + 0.082;
        ...
        //private const double conductivityGrainBoundary = 2.46E1; // Paper: 2.46E9 W/m^2K
        ...
        public static void RunSingleAnalysisAndPlotting()
        {
            ...
            //input.HexagonSize = 0.1;
            //input.NumHexagons = new int[] { 13, 15 };
            //input.HexGridUpperLeftCoords = new double[] 
            //{ 
            //    input.MinCoords[0] - 2.08 * input.HexagonSize,
            //    input.MaxCoords[1] + 1.15 * input.HexagonSize 
            //};
            ...
            input.HexagonSize = 1000; //nm
            input.NumHexagons = new int[] { 25, 25 };
            input.HexGridUpperLeftCoords = new double[]
            {
                input.MinCoords[0] - 2.08 * input.HexagonSize,
                input.MaxCoords[1] + 1.15 * input.HexagonSize
            ...
            var hexGrid = new HexagonalGrid(input.HexagonSize, input.NumHexagons[0], input.NumHexagons[1], 
                input.HexGridUpperLeftCoords[0], input.HexGridUpperLeftCoords[1]);
            GeometricModel geometricModel = new MultigrainPhaseReader().CreatePhasesFromHexagonalGrid(hexGrid);
            var paths = new OutputPaths();
            paths.FillAllForDirectory(@"C:\Users\Serafeim\Desktop\HEAT\Paper\MultigrainHexagonalExample1");
        ...
        public static void RunHomogenization()
        {
            ...
            //input.HexGridUpperLeftCoords = new double[]
            //{
            //    input.MinCoords[0] - 2 * input.HexagonSize - 0.03,
            //    input.MaxCoords[1] + input.HexagonSize + 0.082
            //};
        ...
        private static XModel CreatePhysicalModel(GeometricModel geometricModel, ProblemInput input)
        {
            ...
            // Mesh generation
            var meshGen = new UniformMeshGenerator2D<XNode>(input.MinCoords[0], input.MinCoords[1], input.MaxCoords[0],
                input.MaxCoords[1], input.NumElements[0], input.NumElements[1]);
        ...
        private class ProblemInput
        {
            ...
            public double HexagonSize { get; set; } = double.NaN;

            public double[] HexGridUpperLeftCoords { get; set; }

            public int[] NumHexagons { get; set; }
        }
    }
}
