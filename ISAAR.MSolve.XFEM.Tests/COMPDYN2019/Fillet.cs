        ...
        private static ISolver DefineSolver(FilletBenchmark benchmark, SolverType solverType)
        {
            ...
            else if (solverType == SolverType.FetiDP)
            {
                ...
                if (benchmark.Model.Subdomains.Count == 5)
                {
                    ...
                    cornerNodes[benchmark.Model.GetSubdomain(0)] = new HashSet<INode>();
                    cornerNodes[benchmark.Model.GetSubdomain(1)] = new HashSet<INode>();

                    // Find the 4 corners of the cracked subdomain
                    XSubdomain crackedSubdomain = benchmark.Model.Subdomains[4];
                    double tol = 1E-6;

                    INode[] leftSideNodes = crackedSubdomain.Nodes.Values.Where(node => Math.Abs(node.X - 150.0) <= tol).ToArray();
                    ...
                    INode[] rightSideNodes = crackedSubdomain.Nodes.Values.Where(node => Math.Abs(node.X - 225.0) <= tol).ToArray();
                    ...
                    foreach (INode node in crackedSubdomain.EnumerateNodes())
                    {
                    ...
                    cornerNodes[benchmark.Model.GetSubdomain(2)] = new HashSet<INode>(new INode[] { cornerCrackedBottomLeft, cornerCrackedBottomRight });
                    cornerNodes[benchmark.Model.GetSubdomain(3)] = new HashSet<INode>(new INode[] { cornerCrackedTopLeft, cornerCrackedTopRight });
                    cornerNodes[benchmark.Model.GetSubdomain(4)] = new HashSet<INode>(new INode[]
                    {
