...
namespace ISAAR.MSolve.Solvers.Tests.DomainDecomposition.Dual.GsiFetiDP
{
    public static class Rve3DFetiDP3dTests
    {
        ...
        public static void Run(Mesh mesh, BoundaryConditions bc, Crosspoints crosspoints, Augmentation augmentation)
        {
            ...
            (IVectorView fetiDisplacements, ISolverLogger gsiLogger, ISolverLogger fetiLogger) =
        ...
        private static (IVectorView globalDisplacements, ISolverLogger gsiLogger, ISolverLogger fetiLogger) 
            SolveModelWithSubdomains(Model model, Crosspoints crosspoints, Augmentation augmentation, 
            double pcgConvergenceTolerance)
        {
            ...
            var fetiMatrices = new FetiDP3dMatrixManagerFactorySuiteSparse(new OrderingAmdSuiteSparse());
            var fetiSolverBuilder = new FetiDP3dSolverSerial.Builder(fetiMatrices);
            ...
            FetiDP3dSolverSerial fetiSolver = fetiSolverBuilder.Build(model, cornerNodeSelection, midsideNodesSelection);
            ...
            // Run the analysis
            Utilities.AnalyzeMultiSubdomainModel(model, gsiSolver);

            //gsiSolver.OrderDofs(false);
            //foreach (ISubdomain subdomain in model.EnumerateSubdomains())
            //{
            //    ILinearSystemMpi linearSystem = fetiSolver.GetLinearSystem(subdomain);
            //    linearSystem.Reset(); // Necessary to define the linear system's size 
            //    linearSystem.Subdomain.Forces = Vector.CreateZero(linearSystem.Size);
            //    linearSystem.RhsVector = linearSystem.Subdomain.Forces;
            //}
            //gsiSolver.BuildGlobalMatrix(new ElementStructuralStiffnessProvider());
            //model.ApplyLoads();
            //LoadingUtilities.ApplyNodalLoads(model, fetiSolver);

            //gsiSolver.GlobalForces = Vector.CreateZero(model.GlobalDofOrdering.NumGlobalFreeDofs);
            //foreach (ISubdomain subdomain in model.EnumerateSubdomains())
            //{
            //    model.GlobalDofOrdering.AddVectorSubdomainToGlobal(subdomain, subdomain.Forces, gsiSolver.GlobalForces);
            //}
            //gsiSolver.Solve();

            // Gather the global displacements
            return (gsiSolver.GlobalDisplacements, gsiSolver.Logger, fetiSolver.Logger);
        }
