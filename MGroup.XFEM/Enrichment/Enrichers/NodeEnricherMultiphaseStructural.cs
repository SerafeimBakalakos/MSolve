...
//TODO: Merge this with the one for thermal problems
namespace MGroup.XFEM.Enrichment.Enrichers
{
    public class NodeEnricherMultiphaseStructural : INodeEnricher
    {
        private readonly int dimension;
        private readonly PhaseGeometryModel geometricModel;
        private readonly ISingularityResolver singularityResolver;

        public NodeEnricherMultiphaseStructural(int dimension, PhaseGeometryModel geometricModel, 
            ...
            foreach (IPhase phase in geometricModel.Phases.Values)
            {
                foreach (IPhaseBoundary boundary in phase.ExternalBoundaries)
                {
                    ...
                    if (!enrichmentsExists)
                    {
                        ...
                        var enrichedDofs = new IDofType[dimension];
                        enrichedDofs[0] = new EnrichedDof(enrichmentFunc, StructuralDof.TranslationX);
                        if (dimension >= 2) enrichedDofs[1] = new EnrichedDof(enrichmentFunc, StructuralDof.TranslationY);
                        if (dimension == 3) enrichedDofs[2] = new EnrichedDof(enrichmentFunc, StructuralDof.TranslationZ);

                        enrichmentItem = new EnrichmentItem(id, new IEnrichmentFunction[] { enrichmentFunc }, enrichedDofs);
                        ++id;
