...
namespace MGroup.XFEM.Interpolation.Inverse
{
    ...
    /// </summary>
    public class InverseInterpolationQuad4: IInverseInterpolation
    {
        // Constants
        ...
        private readonly Func<double, double, double[]> formula;

        public InverseInterpolationQuad4(IReadOnlyList<INode> nodes)
        {
            int numOrderings = 4;
        ...
        public double[] TransformPointCartesianToNatural(double[] point)
        {
            ...
            double d1 = 4 * point[0] - sum1;
            double d2 = 4 * point[1] - sum2;

            return formula(d1, d2);
        }

        #region case specific code
        private Func<double, double, double[]> FindQuadDependentFormula()
        {
        ...
        private double[] FormulaForCases123(double d1, double d2)
        {
        ...
        private double[] FormulaForCase4(double d1, double d2)
        {
            double ad = a1 * d2 - a2 * d1;
            double dc = d1 * c2 - d2 * c1;
        ...
        private double[] FormulaForCase5(double d1, double d2)
        {
            double ad = a1 * d2 - a2 * d1;
        ...
        private double[] FormulaForCase6(double d1, double d2)
        {
        ...
        private static IReadOnlyList<INode> CycleCounterClockwise(IReadOnlyList<INode> nodes)
        {
            var cycled = new INode[nodes.Count];
