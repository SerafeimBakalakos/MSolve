...
namespace MGroup.XFEM.Interpolation.Jacobians
{
    public class JacobianShell8Calculations
    {
        ...
        public static (Matrix[] ll1, Matrix[] J_0a) Getll1AndJ_0a(IQuadrature quadrature, double[] tk, 
            IReadOnlyList<double[]> shapeFunctions, IReadOnlyList<Matrix> shapeFunctionDerivatives)
        {
            ...
            for (int j = 0; j < nGaussPoints; j++) //calcualte ll1 as a whole, at each gp
            {
                ...
                for (int k = 0; k < 8; k++)
                {
                    ...
                    ll1[j][0, 3 * k + 1] = 0.5 * gaussPoint.Coordinates[2] * tk[k] * shapeFunctionDerivatives[j][k,0];
                    ll1[j][0, 3 * k + 2] = -ll1[j][0, 3 * k + 1];
                    ll1[j][1, 3 * k] = shapeFunctionDerivatives[j][k,1];
                    ll1[j][1, 3 * k + 1] = 0.5 * gaussPoint.Coordinates[2] * tk[k] * shapeFunctionDerivatives[j][k,1];
                    ll1[j][1, 3 * k + 2] = -ll1[j][1, 3 * k + 1];
                    ll1[j][2, 3 * k] = 0;
