...
namespace MGroup.XFEM.Interpolation
{
    ...
    /// </summary>
    public interface IIsoparametricInterpolation
    {
        /// <summary>
        ...
        /// </summary>
        IReadOnlyList<double[]> NodalNaturalCoordinates { get; }
        ...
        void CheckElementNodes(IReadOnlyList<INode> nodes);

        /// <summary>
        /// The inverse mapping of this interpolation, namely from global cartesian to natural (element local) coordinate system.
        /// </summary>
        /// <param name="nodes">The nodes of the finite element in the global cartesian coordinate system.</param>
        IInverseInterpolation CreateInverseMappingFor(IReadOnlyList<INode> nodes);
        ...
        ///     element.</param>
        EvalInterpolation EvaluateAllAt(IReadOnlyList<INode> nodes, double[] naturalPoint);
        ...
        /// <see cref="EvaluateAllAt(IReadOnlyList{INode}, double[])"/> when the integration points of an element are
        /// the same across multiple elements or multiple iterations of a non linear or dynamic analysis (in the latter cases
        /// we could also cache the <see cref="EvalInterpolation"/> at the integration points of each element). 
        /// The <see cref="EvalInterpolation"/>s per integration point are returned in the same order as the integration 
        ...
        ///     same.</param>
        IReadOnlyList<EvalInterpolation> EvaluateAllAtGaussPoints(IReadOnlyList<INode> nodes, IQuadrature quadrature);
        ...
        /// <param name="naturalPoint">The coordinates of the point in the natural (element local) coordinate system of the 
        ///     element.</param>
        double[] EvaluateFunctionsAt(double[] naturalPoint);

        /// <summary>
        /// Evaluate the shape functions at the integration points defined by a given quadrature. 
        /// This method caches all possible quantities from previous calls. Use it instead of 
        /// <see cref="EvaluateFunctionsAt(double[])"/> when the integration points of an element are the same across 
        ...
        IReadOnlyList<double[]> EvaluateFunctionsAtGaussPoints(IQuadrature quadrature);
        ...
        /// <param name="naturalPoint">The coordinates of the point in the natural (element local) coordinate system of the 
        ///     element.</param>
        Matrix EvaluateNaturalGradientsAt(double[] naturalPoint);

        /// <summary>
        /// Evaluate the 1st order shape function derivatives with respect to the natural coordinate system 
        /// at the integration points defined by a given quadrature. 
        /// This method caches all possible quantities from previous calls. Use it instead of 
        /// <see cref="EvaluateNaturalGradientsAt(double[])"/>, when the integration points of an element 
        ...
        IReadOnlyList<Matrix> EvaluateNaturalGradientsAtGaussPoints(IQuadrature quadrature);
        ...
        ///     element.</param>
        double[] TransformNaturalToCartesian(IReadOnlyList<INode> nodes, double[] naturalPoint);
    }
}