...
namespace MGroup.XFEM.Interpolation
{
    ...
    public class InterpolationHexa27 : IsoparametricInterpolationBase
	{
		...
		private InterpolationHexa27() : base(3, CellType.Hexa27, 27)
		{
		...
		protected sealed override double[] EvaluateAt(double[] naturalPoint)
		{
			...
			values[0] = 1 / 8.0 * xi * (xi - 1) * eta * (eta - 1) * zeta * (zeta - 1);
			values[1] = 1 / 8.0 * xi * (xi + 1) * eta * (eta - 1) * zeta * (zeta - 1);
			values[2] = 1 / 8.0 * xi * (xi + 1) * eta * (eta + 1) * zeta * (zeta - 1);
			values[3] = 1 / 8.0 * xi * (xi - 1) * eta * (eta + 1) * zeta * (zeta - 1);

			values[4] = 1 / 8.0 * xi * (xi - 1) * eta * (eta - 1) * zeta * (zeta + 1);
			values[5] = 1 / 8.0 * xi * (xi + 1) * eta * (eta - 1) * zeta * (zeta + 1);
			values[6] = 1 / 8.0 * xi * (xi + 1) * eta * (eta + 1) * zeta * (zeta + 1);
			values[7] = 1 / 8.0 * xi * (xi - 1) * eta * (eta + 1) * zeta * (zeta + 1);

			values[8] = 1 / 4.0 * (1 - xi * xi) * eta * (eta - 1) * zeta * (zeta - 1);
			values[9] = 1 / 4.0 * xi * (xi - 1) * (1 - eta * eta) * zeta * (zeta - 1);
			values[10] = 1 / 4.0 * xi * (xi - 1) * eta * (eta - 1) * (1 - zeta * zeta);
			values[11] = 1 / 4.0 * xi * (xi + 1) * (1 - eta * eta) * zeta * (zeta - 1);

			values[12] = 1 / 4.0 * xi * (xi + 1) * eta * (eta - 1) * (1 - zeta * zeta);
			values[13] = 1 / 4.0 * (1 - xi * xi) * eta * (eta + 1) * zeta * (zeta - 1);
			values[14] = 1 / 4.0 * xi * (xi + 1) * eta * (eta + 1) * (1 - zeta * zeta);
			values[15] = 1 / 4.0 * xi * (xi - 1) * eta * (eta + 1) * (1 - zeta * zeta);

			values[16] = 1 / 4.0 * (1 - xi * xi) * eta * (eta - 1) * zeta * (zeta + 1);
			values[17] = 1 / 4.0 * xi * (xi - 1) * (1 - eta * eta) * zeta * (zeta + 1);
			values[18] = 1 / 4.0 * xi * (xi + 1) * (1 - eta * eta) * zeta * (zeta + 1);
			values[19] = 1 / 4.0 * (1 - xi * xi) * eta * (eta + 1) * zeta * (zeta + 1);

			values[20] = 1 / 2.0 * (1 - xi * xi) * (1 - eta * eta) * zeta * (zeta - 1);
			values[21] = 1 / 2.0 * (1 - xi * xi) * eta * (eta - 1) * (1 - zeta * zeta);
			values[22] = 1 / 2.0 * xi * (xi - 1) * (1 - eta * eta) * (1 - zeta * zeta);
			values[23] = 1 / 2.0 * xi * (xi + 1) * (1 - eta * eta) * (1 - zeta * zeta);

			values[24] = 1 / 2.0 * (1 - xi * xi) * eta * (eta + 1) * (1 - zeta * zeta);
			values[25] = 1 / 2.0 * (1 - xi * xi) * (1 - eta * eta) * zeta * (zeta + 1);
			values[26] = (1 - xi * xi) * (1 - eta * eta) * (1 - zeta * zeta);
			return values;
		}

		protected override Matrix EvaluateGradientsAt(double[] naturalPoint)
		{
			var x = naturalPoint[0];
			var y = naturalPoint[1];
			var z = naturalPoint[2];
