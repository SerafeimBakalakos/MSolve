...
{
    public class IntersectionMesh3D_NEW : IIntersectionMesh
    {
        private const int dim = 3;

        public IntersectionMesh3D_NEW()
        {
        }

        public static IntersectionMesh3D_NEW CreateMultiCellMesh3D(Dictionary<double[], HashSet<ElementFace_NEW>> intersectionPoints)
        {
            var mesh = new IntersectionMesh3D_NEW();
        ...
        public static IntersectionMesh3D_NEW CreateSingleCellMesh(CellType cellType, IReadOnlyList<double[]> intersectionPoints)
        {
            var mesh = new IntersectionMesh3D_NEW();
        ...
        public static IntersectionMesh3D_NEW JoinMeshes(Dictionary<int, IntersectionMesh3D_NEW> intersectionsOfElements)
        {
            var jointMesh = new IntersectionMesh3D_NEW();
            foreach (IntersectionMesh3D_NEW mesh in intersectionsOfElements.Values)
            {
        ...
        public void MergeWith(IntersectionMesh3D_NEW other)
        {
        ...
        private static List<double[]> OrderPoints3D(Dictionary<double[], HashSet<ElementFace_NEW>> facesOfPoints)
        {
            ...
            while (leftoverPoints.Count > 0)
            {
                ...
                HashSet<ElementFace_NEW> phasesI = facesOfPoints[pointI];
        ...
        private static int FindPointWithCommonFace(HashSet<ElementFace_NEW> phasesI, List<double[]> leftoverPoints, 
            Dictionary<double[], HashSet<ElementFace_NEW>> facesOfPoints)
        {
            for (int j = 0; j < leftoverPoints.Count; ++j)
            {
                HashSet<ElementFace_NEW> phasesJ = facesOfPoints[leftoverPoints[j]];
                if (HaveCommonEntries(phasesI, phasesJ)) return j;
            }
            return -1;
        }

        private static bool HaveCommonEntries(HashSet<ElementFace_NEW> facesSet0, HashSet<ElementFace_NEW> facesSet1)
        {
