...
{
    public class LsmTri3Interaction_NEW  : LsmCellInteractionBase
    {
        public LsmTri3Interaction_NEW(IList<int> nodeIDs, List<double[]> nodeCoords, List<double> nodeLevelSets, double tolerance)
        ...
        private void ProcessCase0Zeros2SameSigns1Different()
        {
            ...
            for (int i = 0; i < 3; ++i)
            {
                ...
                if (nodeLevelSets[i] * nodeLevelSets[j] < 0)
                {
                    ...
                    Mesh.IntersectedEdges.Add(DefineIntersectedEdge(i, j));
                }
            }
