...
namespace MGroup.XFEM.Integ.Quadratures
{
    ...
    /// </summary>
    public sealed class GaussLegendre2D : IQuadrature
    {
        ...
        private GaussLegendre2D(int orderXi, int orderEta)
        {
            ...
            foreach (var pointEta in quadratureEta.IntegrationPoints)
            {
                ...
                {
                    points2D.Add(new GaussPoint(new double[] { pointXi.Coordinates[0], pointEta.Coordinates[0] },
                        pointXi.Weight * pointEta.Weight));
                }
            }
