...
namespace MGroup.XFEM.FEM.Output
{
    ...
    /// </summary>
    public class StrainStressField3D
    {
        private const int numTensorEntries = 6; //TODO: use a dedicated Tensor3D class

        private readonly Dictionary<Node, (double[] strains, double[] stresses)> data;
        private readonly Model model;

        public StrainStressField3D(Model model)
        {
            this.model = model;
        ...
        public void CalculateNodalTensors(IVectorView freeDisplacements)
        {
            ...
            foreach (Subdomain subdomain in model.Subdomains)
            {
                ...
                {
                    ContinuumElement3D elementType = (ContinuumElement3D)(element.ElementType); //TODO: remove cast
