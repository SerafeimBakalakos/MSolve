...
namespace MGroup.XFEM.FEM.Elements
{
	...
	/// the appropriate <see cref="IIsoparametricInterpolation"/>, <see cref="IQuadrature"/> etc. strategies. 
	/// </summary>
	public class ContinuumElement3D : IStructuralFiniteElement, ICell<Node>
	{
		...
		private readonly IReadOnlyList<IContinuumMaterial> materialsAtGaussPoints;

		private double[][] strainsVec;
		private double[][] strainsVecLastConverged;

		public ContinuumElement3D(IReadOnlyList<Node> nodes, IIsoparametricInterpolation interpolation,
			IQuadrature quadratureForStiffness, IQuadrature quadratureForMass,
			IGaussPointExtrapolation gaussPointExtrapolation,
		...
		public IQuadrature QuadratureForConsistentMass { get; }
		public IQuadrature QuadratureForStiffness { get; }
			...
			double[] localdDisplacements)
		{
			...
			{
				var jacobian = new IsoparametricJacobian(3, Nodes, shapeGradientsNatural[gpo]);
				Matrix shapeGradientsCartesian =
		...
		/// <summary>
		/// Calculates strains & stresses at Gauss points, but assumes elastic material.
		/// </summary>
		/// <param name="localDisplacements"></param>
		/// <returns></returns>
		public (IReadOnlyList<double[]> strains, IReadOnlyList<double[]> stresses)
			CalcStrainsStressesAtGaussPoints(double[] localDisplacements)
		{
