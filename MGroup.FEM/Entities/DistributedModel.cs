...
{
    public class DistributedModel : IStructuralModel
    {
        private readonly IComputeEnvironment environment;

        public DistributedModel(IComputeEnvironment environment)
        {
        ...
        {
            environment.DoPerNode(s => SubdomainsDictionary[s].Forces.Clear());
        ...
        public void AssignMassAccelerationHistoryLoads(int timeStep)
        {
            if (MassAccelerationHistoryLoads.Count > 0)
            {
                ...
                {
                    Subdomain subdomain = SubdomainsDictionary[s];
        ...
        public void AssignNodalLoads(NodalLoadsToSubdomainsDistributor distributeNodalLoads)
        {
            ...
            environment.DoPerNode(s =>
            {
                SubdomainsDictionary[s].Forces.AddIntoThis(subdomainNodalLoads[s]);
            });
        }
        ...
        private void AssignConstraints()
        {
            ...
            environment.DoPerNode(s => SubdomainsDictionary[s].ExtractConstraintsFromGlobal(Constraints));
        }
        ...
        private void BuildInterconnectionData()//TODOMaria: maybe I have to generate the constraints dictionary for each subdomain here
        {
            ...
            //environment.DoPerNode(s => SubdomainsDictionary[s].DefineNodesFromElements());
        }

        private void BuildSubdomainOfEachElement()
        {
            foreach (int s in SubdomainsDictionary.Keys)
            {
                Subdomain subdomain = SubdomainsDictionary[s];
