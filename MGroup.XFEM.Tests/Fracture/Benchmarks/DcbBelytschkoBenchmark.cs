...
namespace MGroup.XFEM.Tests.Fracture.Benchmarks
{
    public static class DcbBelytschkoBenchmark
    {
        ...
        private static void ApplyBoundaryConditions(XModel<IXCrackElement> model)
        {
            ...
            XNode topLeft = model.XNodes.Where(n => Math.Abs(n.X) <= tol && Math.Abs(n.Y - H) <= tol).First();
            XNode bottomLeft = model.XNodes.Where(n => Math.Abs(n.X) <= tol && Math.Abs(n.Y) <= tol).First();
            model.NodalLoads.Add(new NodalLoad(topLeft, StructuralDof.TranslationY, +load));
            model.NodalLoads.Add(new NodalLoad(bottomLeft, StructuralDof.TranslationY, -load));
            foreach (XNode node in model.XNodes.Where(n => Math.Abs(n.X - L) <= tol))
            {
