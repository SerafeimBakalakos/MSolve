...
//TODO: Weird geometric bugs with simple and dual mesh LSM
namespace MGroup.XFEM.Tests.MultiphaseThermal.EpoxyAg
{
    public static class Sintering
    {
        private static readonly string outputDirectory = Path.Combine(
            Directory.GetParent(Directory.GetCurrentDirectory()).Parent.Parent.FullName, "Resources", "sintering_2D_temp");
        //private static readonly string expectedDirectory = Path.Combine(
        //    Directory.GetParent(Directory.GetCurrentDirectory()).Parent.Parent.FullName, "Resources", "sintering_2D");

        private static readonly double[] minCoords = { -1.0, -1.0 };
        private static readonly double[] maxCoords = { +1.0, +1.0 };
        private const double thickness = 1.0;
        private static readonly int[] numElementsCoarse = { 20, 20 };
        private static readonly int[] numElementsFine = { 60, 60 };
        ...
        public static void TestModel()
        {
            try
            {
                ...
                // Plot enrichments
                double elementSize = (maxCoords[0] - minCoords[0]) / numElementsFine[0];
        ...
        private static XModel<IXMultiphaseElement> CreateModel()
        {
            ...
            return Models.CreateQuad4Model(minCoords, maxCoords, thickness, numElementsCoarse,
        ...
        private static PhaseGeometryModel CreatePhases(XModel<IXMultiphaseElement> model)
        {
            ...
            List<Circle2D> balls = GenerateBalls();
            for (int p = 0; p < balls.Count; ++p)
            {
                var phase = new LsmPhase(p+1, geometricModel, 0);
                var lsm = new DualMeshLsm2D(phase.ID, mesh, balls[p]);
                geometricModel.Phases[phase.ID] = phase;

                var boundary = new ClosedPhaseBoundary(lsm.ID, lsm, defaultPhase, phase);
        ...
        private static List<Circle2D> GenerateBalls()
        {
            var rng = new Random(seed);
            var balls = new List<Circle2D>();
            for (int i = 0; i < numBalls; ++i)
            {
                double x = minCoords[0] + (maxCoords[0] - minCoords[0]) * rng.NextDouble();
                double y = minCoords[1] + (maxCoords[1] - minCoords[1]) * rng.NextDouble();
                double r = ballRadiusMin + (ballRadiusMax - ballRadiusMin) * rng.NextDouble();
                balls.Add(new Circle2D(x, y, r));
            }
            return balls;
        }
    }
