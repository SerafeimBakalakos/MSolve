...
{
    public static class DualMeshLsmUnionBalls3DTests
    {
        private static readonly string outputDirectory = Path.Combine(
            Directory.GetParent(Directory.GetCurrentDirectory()).Parent.Parent.FullName, "Resources", "dual_mesh_union_balls_3D_temp");
        private static readonly string expectedDirectory = Path.Combine(
            Directory.GetParent(Directory.GetCurrentDirectory()).Parent.Parent.FullName, "Resources", "dual_mesh_union_balls_3D");
        ...
        private static PhaseGeometryModel CreatePhases(XModel<IXMultiphaseElement> model, DualMesh3D mesh)
        {
            ...
            for (int p = 0; p < balls.Length; ++p)
            {
                ...
                var curve = new DualMeshLsm3D(phase.ID, mesh, balls[p]);
