...
{
    public static class DualMeshUnionHollowBalls2DTests
    {
        private static readonly string outputDirectory = Path.Combine(
            Directory.GetParent(Directory.GetCurrentDirectory()).Parent.Parent.FullName, "Resources", "dual_mesh_union_hollow_balls_2D_temp");
        private static readonly string expectedDirectory = Path.Combine(
            Directory.GetParent(Directory.GetCurrentDirectory()).Parent.Parent.FullName, "Resources", "dual_mesh_union_hollow_balls_2D");

        private static readonly double[] minCoords = { -1.0, -1.0 };
        private static readonly double[] maxCoords = { +1.0, +1.0 };
        private const double thickness = 1.0;
        private static readonly int[] numElementsCoarse = { 15, 15 };
        private static readonly int[] numElementsFine = { 75, 75 };
        ...
        public static void TestModel()
        {
            try
            {
                ...
                XModel<IXMultiphaseElement> model = Models.CreateQuad4Model(mesh.CoarseMesh,
        ...
        private static PhaseGeometryModel CreatePhases(XModel<IXMultiphaseElement> model, DualMesh2D mesh)
        {
            ...
            for (int p = 0; p < ballsInternal.Length; ++p)
            {
                ...
                var externalCurve = new DualMeshLsm2D(externalPhase.ID, mesh, ballsExternal[p]);
                ...
                var internalLsm = new DualMeshLsm2D(2 * p + 2, mesh, ballsInternal[p]);
