...
{
    public static class DualMeshLsmUnionBalls2DTests
    {
        private static readonly string outputDirectory = Path.Combine(
            Directory.GetParent(Directory.GetCurrentDirectory()).Parent.Parent.FullName, "Resources", "dual_mesh_union_balls_2D_temp");
        private static readonly string expectedDirectory = Path.Combine(
            Directory.GetParent(Directory.GetCurrentDirectory()).Parent.Parent.FullName, "Resources", "dual_mesh_union_balls_2D");
        ...
        private static PhaseGeometryModel CreatePhases(XModel<IXMultiphaseElement> model, DualMesh2D mesh)
        {
            ...
            for (int p = 0; p < balls.Length; ++p)
            {
                ...
                var curve = new DualMeshLsm2D(phase.ID, mesh, balls[p]);
