...
{
    public static class DualMeshUnionHollowBalls3DTests
    {
        private static readonly string outputDirectory = Path.Combine(
            Directory.GetParent(Directory.GetCurrentDirectory()).Parent.Parent.FullName, "Resources", "dual_mesh_union_hollow_balls_3D_temp");
        private static readonly string expectedDirectory = Path.Combine(
            Directory.GetParent(Directory.GetCurrentDirectory()).Parent.Parent.FullName, "Resources", "dual_mesh_union_hollow_balls_3D");

        private static readonly double[] minCoords = { -1.0, -1.0, -1.0 };
        private static readonly double[] maxCoords = { +1.0, +1.0, +1.0 };
        private static readonly int[] numElementsCoarse = { 10, 10, 10 };
        private static readonly int[] numElementsFine = { 50, 50, 50 };
        ...
        public static void TestModel()
        {
            try
            {
                ...
                XModel<IXMultiphaseElement> model = Models.CreateHexa8Model(mesh.CoarseMesh, bulkIntegrationOrder,
                    boundaryIntegrationOrder, materialField);
        ...
        private static PhaseGeometryModel CreatePhases(XModel<IXMultiphaseElement> model, DualMesh3D mesh)
        {
            ...
            for (int p = 0; p < ballsInternal.Length; ++p)
            {
                ...
                var externalCurve = new DualMeshLsm3D(externalPhase.ID, mesh, ballsExternal[p]);
                ...
                var internalLsm = new DualMeshLsm3D(2 * p + 2, mesh, ballsInternal[p]);
