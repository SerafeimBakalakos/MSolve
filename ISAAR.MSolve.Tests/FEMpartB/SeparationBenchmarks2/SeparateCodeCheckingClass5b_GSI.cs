...
{
    public static class SeparateCodeCheckingClass5b_GSI
    {//check unpushed changes commit
        ...
        static void Solve()
        {
            for (int example = 28; example < 29; example++)
            {
                ...
                (Model model1, double[] uc1, Vector globalU1, bool IsFetiDpSolver3d) = RunExample();
                (Model model2, double[] uc2, Vector globalU2) = RunExampleSerial();
        ...
        public static (Model, double[], Vector, bool) RunExample()
        {
            ...
            RunAnalysis(model, gsiSolver); // apo to paradeigma ISAAR.MSolve.Solvers.Tests.DomainDecomposition.Dual.FetiDP.IntegrationTests.FetiDPSolverSerialTests.RunAnalysis()
            #endregion
